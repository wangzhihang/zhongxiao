appControllers.controller('kuandai-submit-wojia', function($scope, $state, $rootScope, $http, $ionicPopup, $timeout, unicomm_server, ble) {
	...
	$scope.uc.baseinfo = {
		...
		"fee_8234":String(kuandai_combination.fee_8234-kuandai_combination.fee_8234_jm),
		"fee_8376":String(kuandai_combination.fee_8376)
	};
	$scope.wojieCmd();
	$scope.uc.iptvinfo = {};
	...
	$scope.getacctid = function(){
		$scope.domEaplan = "获取宽带账号信息("+$scope.current_kuandai+")";
		$scope.domLine = "20";
		unicomm_server.getUnicomm({
	...
	$scope.kuandai_number = kuandai_combination_list.length;
	$scope.current_kuandai = 1;
	$scope.getlanacctinfo = function(){
			unicomm_server.getUnicomm({
				"cmd":"cbss_lan_getlanacctinfo",
				"accessType":kuandai_combination_address["detailed"]["accessType"],
				"productId":kuandai_combination_list[$scope.current_kuandai-1].mainLanProductList['productId'],
				"brandCode":"GZKD",
	...
	$scope.uc.mainLanProductList = {}
	$scope.uc.lanProductList = {}
	$scope.uc.lanElementList = {}
	$scope.uc.addressInfoList = {}
	$scope.uc.laninfoList = []
	$scope.kuandai_number = kuandai_combination_list.length;
	$scope.current_kuandai = 1;
	$scope.kuandai_info = function(){
		$scope.uc.mainLanProductList[$scope.uc.baseinfo["authAcctId"]] = 
		kuandai_combination_list[$scope.current_kuandai-1].mainLanProductList;
		$scope.uc.lanProductList[$scope.uc.baseinfo["authAcctId"]] = 
		kuandai_combination_list[$scope.current_kuandai-1].lanProductList.concat(kuandai_combination_list[$scope.current_kuandai-1].lanProductListTv);
		$scope.uc.lanElementList[$scope.uc.baseinfo["authAcctId"]] = 
		kuandai_combination_list[$scope.current_kuandai-1].lanElementList.concat(kuandai_combination_list[$scope.current_kuandai-1].lanElementListTv);
		$scope.uc.addressInfoList[$scope.uc.baseinfo["authAcctId"]] = 
		kuandai_combination_list[$scope.current_kuandai-1].addressInfoList;
		
		$scope.uc.laninfoList.push(kuandai_combination_list[$scope.current_kuandai-1].lanInfoList);
		$scope.uc.laninfoList[$scope.uc.laninfoList.length-1].authAcctId = $scope.uc.baseinfo["authAcctId"];
		$scope.uc.laninfoList[$scope.uc.laninfoList.length-1].iptvid = $scope.uc.iptvinfo["iptvid"];
		
		if($scope.kuandai_number == $scope.current_kuandai){
			$scope.uploadImage();
		}else{
			$scope.current_kuandai++;
			$scope.getlanacctinfo();
		}
	}
	...
	$scope.seveOrder = function(){
		...
		$scope.lanInfoList = [];
		for(var i in $scope.uc.mainLanProductList){
			var laninfoList_temp = {}
			for(var ii in $scope.uc.laninfoList){
				if($scope.uc.laninfoList[ii].authAcctId == i){
					laninfoList_temp = $scope.uc.laninfoList[ii];
				}
			}
			$scope.lanInfoList.push({
				...
				"lanAddress":kuandai_combination_address["setupaddress"], //laninfoList_temp["setupaddress"],	//(宽带地址)
				"addressCode":kuandai_combination_address["addressCode"],	//$scope.uc.addressInfoList[i]["addressId"],	//(地址编码)

				"comment":kuandai_combination["remark"],					//(备注信息)

				"detailedProductId":$scope.uc.mainLanProductList[i]["productId"],		//(子套餐ID) 50M||100M
				"detailedProductName":$scope.uc.mainLanProductList[i]["productName"],	//(子套餐名称)
				"lanAmount":laninfoList_temp["preCharge"],			// 宽带费用
				"lanNumber":i,	//(宽带号码)

				"ftthAddress":"",		//(MAC地址)
				"ftthPassword":"",		//(TV密码)
				"ftthType":laninfoList_temp["terminaltype"], 	//(光猫类型)
				"ftthAmount":laninfoList_temp["gateWayFee"], 	//(光猫价格)

				"macAddress":laninfoList_temp["macaddress"],	//(MAC地址)
				"tvPassword":"",		//(TV密码)
				"tvType":laninfoList_temp["feetype"], 			//(机顶盒类型)
				"tvAmount":laninfoList_temp["telBoxFee"], 		//(机顶盒价格)
				"account":laninfoList_temp["iptvid"],			//(TV账户)
			})
		}
		...
		if(isEmptyObject($scope.inputData)){
			...
			$scope.inputData = {
				...
				"lanInfoList":JSON.stringify($scope.lanInfoList),
