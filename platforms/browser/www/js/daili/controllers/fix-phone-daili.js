appControllers.controller('fixPhoneDaili', function($scope,$http,$rootScope,$state,my,$ionicLoading,$ionicPopup,$filter,$timeout) {
	$scope.title = '固话订单列表';
		...
		setCurrent: function (param) {
			...
			switch($scope.tag.current ){
				...
				default:
					$scope.orderList = [];
					$scope.pageIndex = 1;
					console.log('111=='+$scope.statusArray);
	...
	$scope.telSearch = function(){
		...
		var temp = '<div class="formList f-14">'+
						'<ul>'+
							'<li class="rowItem" style="padding:.5rem;">'+
								'<div class="labelForm">'+
									'<input type="tel" class="textBox" ng-model="input.phonenumber" ng-keyup="telChange()" placeholder="输入8位固话号码或11位长号号码"/>'+
								'</div>'+
							'</li>'+
						'</ul>'+
					'</div>';
				...
				},
			   {
				 ...
				 onTap: function() {
					if($scope.input.phonenumber == '' || $scope.input.phonenumber.length < 7){
					   my.alert('请正确输入手机号码或固话号码').then(function(){
					   		$scope.input.phonenumber = '';
					   });
				   }else{
					   $scope.orderList = [];
	...
	$scope.orderDetail = function(index){
		...
		$scope.numberOrderCode=$scope.orderList[index].numberOrderCode;
		localStorage.setItem('orderCode',$scope.orderList[index].orderCode);
		localStorage.setItem('numberOrderCode',$scope.orderList[index].numberOrderCode);
		$state.go("fixPhoneDailiDetail");
	};
})
