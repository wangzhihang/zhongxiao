appControllers.controller('dianpu-qrgo-applyOrder', function($scope, $state, $http, $rootScope, my, unicomm_server, url2base64) {
...
.controller('dianpu-qrgo-writesimok', function($scope, $state, $http, $rootScope, $ionicActionSheet, ble, unicomm_server, my) {
	...
	$scope.readBtn 		= [false, true];	// 读卡|扫描
	...
	$scope.saveFailed = function(){
		$http({
			...
			data : {"orderCode":authentication["orderNo"], "node":arguments[0]+"(v1)", "failedReason":JSON.stringify(arguments[1]), "equipment":device.platform}
		})
	}

	
	$scope.domEaplan = "订单激活";
	$scope.domLine = "15";
	...
	$scope.qrgo_getCardData = function(i)
	{
		$scope.domEaplan = "提交SIM卡信息!";
		$scope.domLine = "30";
		unicomm_server.getUnicomm({
			"cmd":"qrgo_getCardData",
			"orderId":qrgoInfo.orderId,
			...
			function(return_json){
				if(return_json.status == '1') {
					...
					$scope.saveFailed("2i产品-提交SIM卡信息", result_json.data);
	...
	{
		$scope.domEaplan = "实名制认证中";
		$scope.domLine = "75";

		unicomm_server.getUnicomm({
			"cmd":"qrgo_savePersonInfo",
			"orderId":qrgoInfo.orderId,
			"name":authentication["name"],
			"psptId":authentication["cardId"],
			"nation":authentication["nation"],
			"validDate":authentication["end_date"].replace(/[^\d]/g, ""),
			"startDate":authentication["start_date"].replace(/[^\d]/g, ""),
			"address":authentication["address"],
			"headPicStr":authentication["idHeadImg"],
			"handCardStr":authentication["customerImagebase64"],
			"headWaterMarkPicStr":authentication["customerImagebase64"],
