appControllers.controller('scan-code-order', function($scope,$http,my,$rootScope,$state, $timeout,$ionicPopup) {
	...
	// 取消订单接口
	$scope.giveUp = function(id,orderCode,number,productId,productName,remark){
		$http({
			method:'get',
                 ...
                 onTap: function(e) {
                   if (!$scope.input.txt) {
                     ...
                     e.preventDefault();
                        ...
                        $scope.giveUp($scope.id,$scope.orderCode,$scope.number,$scope.productId,$scope.productName,$scope.remark);
